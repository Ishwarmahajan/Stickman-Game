...
function setup() {
  ...
  stickman.debug = true
  stickman.setCollider("rectangle", 0, 0, 50, 270)
  ...
  friend.debug = true
  friend.setCollider("rectangle", 0, 0, 50, 260)

  boss = createSprite(1100, 250)
  boss.addImage("Standing", boss_i)
  boss.visible = false
  boss.setCollider("rectangle", 10, 0, 120, 350)
  ...
  bullet = createSprite(500, 400, 10, 10)
  bullet.scale = 0.2
  ...
  if(frameCount > 100) {
    ...
    if(keyDown("space")){
      if(destroy === 0 || bullet.lifetime === 0){
        bullet = createSprite(500, 350, 10, 10)
        bullet.scale = 0.2
        bullet.addImage(bullet_i)
        //bullet.visible = false
      }
    ...
    if(boss.isTouching(bullet)){
      scoreB = scoreB-1
      bullet.destroy()
    }

    if(stickman.isTouching(firecharge)){
      scoreS = scoreS-1
      firecharge.destroy()
    }
    ...
    if(scoreB === 0){
      boss.visible = false
      textSize(50)
      text("You Won", 760, 250)
    }

    //if(stickmangun.isTouchng(firecharge)){
    //  scoreS = scoreS-1
    //}
